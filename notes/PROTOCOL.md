# ResoniteLink protocol notes (local)

These notes summarize the current protocol shape as observed in the upstream ResoniteLink repository.

## Transport
- WebSocket
- Payloads are JSON text frames.
- The upstream client library currently does **not** support binary frames.
...
The upstream C# client always ensures `messageId` is present; the response includes `sourceMessageId`.
...
- Resonite-allocated IDs are prefixed with `Reso_` (don’t use this prefix for your own IDs).
...
- `position`, `scale`: `float3` fields (`{ "$type": "float3", "value": {"x":0,"y":0,"z":0} }`)
- `rotation`: `floatQ` field (quaternion-like value)
- `components`: array of `Component` (either fully populated or reference-only depending on query)
- `children`: array of `Slot` (either fully populated or reference-only depending on query)
...
- Vector-like fields: `{ "$type": "float3", "value": {"x":...,"y":...,"z":...} }`
- References: `{ "$type": "reference", "targetId": "..." }`

Upstream defines many `$type` values in `ResoniteLink/Models/DataModel/PrimitiveContainers.cs` and related primitive structs.

## Commands (currently present upstream)
Slots:
...
For `updateSlot` / `updateComponent`:
- Properties you omit (or leave `null` in strongly typed clients) are left unchanged server-side.
